...
class FileMonitor(
  ...
  implicit bf: fauna.tool.ast.Builder[org.json4s.JValue]
...
class LogsMonitor(config: LogsMonitorConfig) {
  ...
  val codegen: Generator = config.codegen match {
    ...
    case "curl" =>
      new fauna.tool.codegen.CurlGenerator(new fauna.tool.ast.JsonSerializer)
  }

  implicit val bf = new fauna.tool.parser.JsonBuilder()
  fauna.tool.ast.Expr.init
